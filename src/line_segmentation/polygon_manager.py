...
def get_polygons_from_lines(img, lines, connected_components, vertical):
    ...
    # Extracting contours for every CC.
    polygon_coords = []
    for i, line in enumerate(lines):
        ...
        for c in line:
            ...
            # Adding the first contour point to the list s.t. the line will be connected.
        ...
        polygon_coords.append(measure.find_contours(polygon_img, 5, fully_connected='high')[0])
...
def find_graph_node(coords, centroid):
    # Casting centroid coordinates into int.
...
def draw_polygons(image, polygons, vertical):
    ...
    for polygon in polygons:
        cv2.polylines(image, np.array([[[np.int(p[1]), np.int(p[0])] for p in polygon]]), 
                      1, color=(248, 24, 148), thickness=3)
    return image
