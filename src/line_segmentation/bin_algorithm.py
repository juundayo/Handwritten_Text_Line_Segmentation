...
def majority_voting(connected_components, seams):
    ...
    # Getting the centroids and sorting them.
    ...
    while len(small_bins) > 0:
        # Splitting values into bins index.
        bin_index, bin_size, unique_bins = split_into_bins_and_index(values)

        # Looking for outliers and merging them into bigger clusters.
        if small_bins[0] == 42:
...
def merge_small_bins(bin_index, centroids, small_bins, values):
    for bin in small_bins:
        # Finding the indices of the samples which are undernumbered.
        for loc in np.where(bin_index == bin)[0]:
            # Looking for the next available bin below.
            ...
            # Looking for the next available bin above.
            ...
            # Computing distances to neighbors with the EUC distance.
...
def split_into_bins_and_index(values):
    ...
    # Getting the bins values and their size.
...
def compute_avg_pairwise_distance(centroids):
    # Sorting the centroids based on their x-coordinate.
    centroids = centroids[np.argsort(centroids[:, 1]), :]
    dist = []
    for c1, c2 in pairwise(centroids):
        # Computing the distance in the horizontal axis.
...
def check_for_anomaly(centroids, threshold):
    ...
    centroids = centroids[np.argsort(centroids[:, 1]), :]
    for c1, c2 in pairwise(centroids):
        # Computing the distance in the horizontal axis; If it is higher thant a threshold, trigger.
...
def draw_bins(img, centroids, root_output_path, seams, bins):
    ...
    # Drawing the centroids on the seam energy map.
